...
def read_path(path_name,son_path_name):#读取路径下的数据集，并为每张图片添加一个类别标签
    ...
    if exit_code == 1:#如果有，则全路径是当前路径上一级＋测试集文件夹名
        ...
        for dir_item in os.listdir(fullpath):#遍历所有的目录
            print dir_item
            fullsubpath = fullpath+'/'+dir_item+'/'
            print fullsubpath
            for image_item in os.listdir(fullsubpath):
                if image_item.endswith('.jpg'):#如果格式是图片，则进行大小处理
                    image = cv2.imread(fullsubpath+'/'+image_item)
                    image = resize_image(image, IMAGE_SIZE, IMAGE_SIZE)

                    images.append(image)
                    labels.append(int(dir_item))
    print labels
